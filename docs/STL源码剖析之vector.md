...
```
`_Vector_base`专门负责`vector`的内存管理，内部类`_M_impl`通过继承`_Tp_alloc_type`(也就是`allocator`)得到内存分配释放的功能，`_M_allocate和_M_deallocate`分别分配和释放`vector`所用内存，`vector`只需要负责元素构造和析构。
`vector`代码中使用基类的内存函数及`typedef`等：
```cpp
